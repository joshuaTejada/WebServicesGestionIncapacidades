            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    if (data.Rows[0]["code"].ToString() == "1")
                    {
                        ...
                        responseModels.Base64ImgEps = GetEPSBase64(data.Rows[0]["codigo"].ToString());
                    }
                    else
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    if (data.Rows[0]["code"].ToString() == "1")
                    {
                        ...
                        string DoumentDetails = "los siguientes documentos no subieron: ";

                        foreach (var (base64, id) in attachments)
                        {
                            if (!string.IsNullOrEmpty(base64))
                            {
                                var (IsValid, Message) = SaveDocument(base64, $"{data.Rows[0]["id_empresa"]}_{data.Rows[0]["id_incapacidad"]}_{id}.pdf");
                                if (!IsValid)
                                {
