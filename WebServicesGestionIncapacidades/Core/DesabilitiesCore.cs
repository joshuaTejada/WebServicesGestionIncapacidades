...
namespace WebServicesGestionIncapacidades.Core
{
    public class DesabilitiesCore
    {
        ...
        public DisabilityResponse PostDisabilities(string Token, DisabilitiesRequest disabilitiesRequest)
        {
            ...
            try
            {
                ...
                foreach (var (file, id) in attachments)
                {
                    ...
                    {
                        responseModels.MessageResponse = $"{validationError} (Adjunto: {id})";
                        responseModels.CodeResponse = "406"; // No Aceptable
                        return responseModels;
                    }
                }
                ...
                foreach (var (file, id) in attachments)
                {
                    ProcessAndSaveFile(file, id_empresa, id_incapacidad, id);
                }
        ...
        private string ValidateFile(IFormFile file)
        {
            ...
            if (fileBytes.Length < 5) return "El archivo es inválido o está corrupto.";

            // Se toma una pequeña parte del arreglo para identificar el tipo de archivo.
            ...
            {
                return "El archivo no es un PDF ni una imagen válida";
            }
            return null;
        }
